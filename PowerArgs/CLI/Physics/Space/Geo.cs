...
namespace PowerArgs.Cli.Physics
{
    public struct Angle
    {
        ...
        public Angle RoundAngleToNearest(Angle nearest) => new Angle(((float)Geo.Round(Value / nearest.Value) * nearest.Value) % 360);
    ...
    public struct RectF
    {
        ...
        public RectF Round() => new RectF(Geo.Round(Left), Geo.Round(Top), Geo.Round(Width), Geo.Round(Height));
        ...
        public static float CalculateNormalizedDistanceTo(float ax, float ay, float aw, float ah, float bx, float by, float bw, float bh)
        {
            ...
            return Geo.NormalizeQuantity(d, a, true);
        }
    ...
    public struct LocF : IEquatable<LocF>
    {
        ...
        public LocF GetRounded() => new LocF(Geo.Round(Left), Geo.Round(Top));
        ...
        public static LocF OffsetByAngleAndDistance(float x, float y, Angle angle, float distance, bool normalized = true)
        {
            if (normalized)
            {
                distance = Geo.NormalizeQuantity(distance, angle.Value);
            }
        ...
        public static float CalculateNormalizedDistanceTo(float ax, float ay, float bx, float by)
        {
            ...
            return Geo.NormalizeQuantity(d, a.Value, true);
        }
